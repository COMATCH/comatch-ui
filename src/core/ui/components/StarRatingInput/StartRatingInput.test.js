...
describe('Start Rating Input test', () => {
    ...
    describe('verifying the HTML structure', () => {
        ...
        it('should render with `readOnly=true` correctly', () => {
            const startRatingInputComponent = mount(<StarRatingInput {...props} readOnly={true}/>);
            expect(startRatingInputComponent.find('div').at(0).hasClass('StarRatingInput read-only')).toBe(true);
        });
    });
